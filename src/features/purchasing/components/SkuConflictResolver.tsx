...
interface SkuConflictResolverProps {
  ...
  onResolutionChange: (rowKey: string, resolution: ConflictResolution) => void
  onConfirm: () => void
  isPending: boolean
}
  ...
  return (
    <>
      ...
      {conflicts.map((conflict) => {
        const resolution = resolutions[conflict.row_key]
        return (
          <div key={conflict.row_key} className="border rounded p-3 space-y-1">
            ...
            <div className="flex gap-4">
              <label className="flex items-center gap-1.5 text-sm cursor-pointer">
                <input type="radio"
                  ...
                  onChange={() => onResolutionChange(conflict.row_key, { action: 'add_to_existing', product_id: conflict.existing_product_id })} />
                Add to &ldquo;{conflict.existing_product_name}&rdquo;
              </label>
              <label className="flex items-center gap-1.5 text-sm cursor-pointer">
                <input type="radio"
                  checked={resolution?.action === 'skip'}
                  onChange={() => onResolutionChange(conflict.row_key, { action: 'skip' })} />
                Skip
              </label>
            </div>
          </div>
